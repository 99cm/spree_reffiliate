...
RSpec.describe Spree::Promotion::Rules::AffiliatedPromotionRule, type: :model do
  before do
    @affiliate = FactoryBot.create(:affiliate, path: 'corona')
    @affiliated = FactoryBot.create(:user, email:  FFaker::Internet.email, affiliate_code: @affiliate.path)
    @affiliated_order = FactoryBot.create(:order, user_id: @affiliated.id)

    @falsy_affiliate = FactoryBot.create(:affiliate, path: 'victoria')
    @falsy_affiliated = FactoryBot.create(:user, email:  FFaker::Internet.email, affiliate_code: @falsy_affiliate.path)
    @falsy_affiliated_order = FactoryBot.create(:order, user_id: @falsy_affiliated.id)

    @user = FactoryBot.create(:user, email: FFaker::Internet.email)
    @organic_order = FactoryBot.create(:order, user_id: @user)

    @rule = FactoryBot.build(:affiliated_promotion_rule)
