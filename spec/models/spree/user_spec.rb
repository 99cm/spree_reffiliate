...
RSpec.describe User, type: :model do
  before(:each) do
    @user = FactoryBot.create(:user, email: FFaker::Internet.email, referral_credits: 50, referrer_benefit_enabled: true)
    @referred = FactoryBot.create(:user, email:  FFaker::Internet.email, referral_code: @user.referral.code, referral_credits: 50, referrer_benefit_enabled: true)
